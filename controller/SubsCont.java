...
public class SubsCont extends Attributes {
    ...
    public String SubsMy(Model model) {
        ...
        if (getUser().getRole() == Role.CLIENT) {
            model.addAttribute("purchsed", getUser().getPurchased());
        } else {
            model.addAttribute("purchsed", purchasedRepo.findAll());
        }
    ...
    public String chat(Model model, @PathVariable Long id) {
        AddAttributes(model);
        model.addAttribute("purchased", purchasedRepo.getReferenceById(id));
        return "subs_chat";
    }

    @PostMapping("/{id}/chat/add")
    public String add(@RequestParam String text, @PathVariable Long id) {
        chatRepo.save(new Chat(text, getUser(), purchasedRepo.getReferenceById(id)));
        return "redirect:/subs/{id}/chat";
    }
